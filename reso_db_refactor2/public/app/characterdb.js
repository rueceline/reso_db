...
async function loadSideTags() {
  const tagJson = await fetchJson(addCacheBust(DATA_URL_TAG, CACHE_BUST));
...
async function loadUnitAndView() {
  const unitJson = await fetchJson(addCacheBust(DATA_URL_UNIT, CACHE_BUST));
  const viewJson = await fetchJson(addCacheBust(DATA_URL_VIEW, CACHE_BUST));
...
async function loadFallbackHomeCharacter() {
  const json = await fetchJson(addCacheBust(DATA_URL_FALLBACK, CACHE_BUST));
