// ---- globals ----
var RSBase = (typeof window !== "undefined" && window.RSBase) ? window.RSBase : {};
var RSUtils = (typeof window !== "undefined" && window.RSUtils) ? window.RSUtils : {};
var RSEquipment = (typeof window !== "undefined" && window.RSEquipment) ? window.RSEquipment : {};

var getAppBase = RSBase.getAppBase;
var withBase = RSBase.withBase;
var addCacheBust = RSBase.addCacheBust;
var fetchJson = RSBase.fetchJson;
var normalizeRootJson = RSBase.normalizeRootJson;
var buildIdMap = RSBase.buildIdMap;
var normalizePathSlash = RSBase.normalizePathSlash;

var uniqSorted = RSUtils.uniqSorted;
var isInvalidGroupValue = RSUtils.isInvalidGroupValue;
var loadFactory = RSUtils.loadFactory;

var parseIdCN = RSEquipment.parseIdCN;
var mapQualityToRarity = RSEquipment.mapQualityToRarity;
var calcMainStatWithGrowth = RSEquipment.calcMainStatWithGrowth;
...
// 장비 / 성장 / 스킬 데이터 경로
const DATA_URL = withBase('public/data/KR/EquipmentFactory.json');
const GROWTH_URL = withBase('public/data/KR/GrowthFactory.json');
const SKILL_URL = withBase('public/data/KR/SkillFactory.json');

// 무기 이미지 에셋 베이스 경로
const ASSET_SMALL_BASE = withBase('public/assets/item/weapon');
...
async function load() {
  ...
  try {
    const [eqJson, growthJson, skillJson] = await Promise.all([
      fetchJson(addCacheBust(DATA_URL, CACHE_BUST)),
      fetchJson(addCacheBust(GROWTH_URL, CACHE_BUST)),
      fetchJson(addCacheBust(SKILL_URL, CACHE_BUST)),
    ]);
