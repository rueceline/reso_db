...
function safeNumber(v, fallback) {
  const b = rsBase();
  return b.safeNumber ? b.safeNumber(v, fallback) : (Number.isFinite(Number(v)) ? Number(v) : (fallback ?? 0));
}
function normalizePathSlash(path) {
  const b = rsBase();
  return b.normalizePathSlash ? b.normalizePathSlash(path) : String(path||"").replaceAll("\\","/");
}
...
function calcEquipStatRange(baseValue, growthList) {
...
function calcMainStatWithGrowth(baseValue, growth, level) {
  const base = safeNumber(baseValue, 0);
...
function getFixedOptionMax(option) {
  if (!option) return 0;
...
(function () {
  ...
  window.RSEquipment = window.RSEquipment || {};
  window.RSEquipment.parseIdCN = parseIdCN;
  window.RSEquipment.mapQualityToRarity = mapQualityToRarity;
  window.RSEquipment.resolveMainStat = resolveMainStat;
  window.RSEquipment.calcEquipStatRange = calcEquipStatRange;
  window.RSEquipment.calcMainStatWithGrowth = calcMainStatWithGrowth;
  window.RSEquipment.getFixedOptionMax = getFixedOptionMax;
})();
